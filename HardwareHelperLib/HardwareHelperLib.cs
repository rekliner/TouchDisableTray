...
namespace HardwareHelperLib
{
    ...
    public class HH_Lib
    {
        ...
        public List<DEVICE_INFO> GetAll(string nameFilter = null, string HidFilter = null)
        {
            List<DEVICE_INFO> HWList = new List<DEVICE_INFO>();
            try
            {
                Guid HID = new Guid("745a17a0-74d3-11d0-b6fe-00a0c90f57da");    // HIDClass GUID
                IntPtr hDevInfo = Native.SetupDiGetClassDevs(ref HID, 0, IntPtr.Zero, Native.DIGCF_PRESENT);
                ...
                for (i = 0; Native.SetupDiEnumDeviceInfo(hDevInfo, i, DeviceInfoData); i++)
                {
                    ...
                    // Filter based on input if present
                    if (nameFilter != null && !DeviceName.ToString().StartsWith(nameFilter))
                    {
                        // nameFilter present, but didn't match, go onto next device
                        continue;
                    }
                    if (HidFilter != null && !DeviceName.ToString().StartsWith(HidFilter))
                    {
                        // nameFilter present, but didn't match, go onto next device
                        continue;
                    }
